...
void calculate_trapezoid_for_block(block_t *block, float entry_factor, float exit_factor) 
{
  ...
  int32_t accelerate_steps =   ceil(estimate_acceleration_distance(block->initial_rate, block->nominal_rate,(float)acceleration));//向上取整
  int32_t decelerate_steps =   floor(estimate_acceleration_distance(block->nominal_rate, block->final_rate, -(float)acceleration));//向下取整
...
#define MAX_FREQ_TIME (1000000.0/XY_FREQUENCY_LIMIT)
  ...
  //	SERIAL_ECHO_START
  //    DBG("After:\r\n") ;
//	  DBG("step_event_count :%d\n",block->step_event_count);
//	  DBG("accelerate_until :%d\n",block->accelerate_until);
...
void allow_cold_extrudes(u8 allow)
{
#ifdef PREVENT_DANGEROUS_EXTRUDE
...
/****************************************************************
* @FunName	: Plan_GetBlocksInBuf()
* @Brief	: Get block number in the buffer.
* @Para	: None.
* @Return	: Block's number.
****************************************************************/
uint8_t Plan_GetBlocksInBufFromInterrupt(void)
{
	uint8_t tmpTail = 0;

	
	tmpTail = block_buffer_tail;
	

	return (block_buffer_head - tmpTail + BLOCK_BUFFER_SIZE) & (BLOCK_BUFFER_SIZE - 1);
}
