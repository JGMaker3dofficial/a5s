...
//SDIO IO口初始化
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOF|RCC_APB2Periph_GPIOD | RCC_APB2Periph_GPIOC,ENABLE);//使能PORTC,PORTD时钟
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;	//PF10 上拉输入,SD卡插入检测
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 
  GPIO_Init(GPIOF, &GPIO_InitStructure);					 //根据设定参数初始化PF10
...
RCC_AHBPeriphClockCmd(RCC_AHBPeriph_SDIO|RCC_AHBPeriph_DMA2,ENABLE);//使能SDIO,DMA2时钟	 
  ...
  if(errorstatus==SD_OK)
		...
		//DEBUG_PRINTF("file:%s,line:%d ->cardType:%d\r\n",__FILE__,__LINE__,SDCardInfo.CardType);
  		}
...
/*
*********************************************************************************************************
*	函数名: is_sdCard_plugIn
*	检测SD卡是否插入: SD Card????
*	输入参数: 无
*	输出参数: 1--已插入,0--未插入
*********************************************************************************************************
*/
u8 is_sdCard_plugIn(void)
{
	u8 retvlue = 0;
	if(SD_CD == 0)
	{
