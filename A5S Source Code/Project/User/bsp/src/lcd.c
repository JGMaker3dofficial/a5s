...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if(lcddev.id==0X9320)opt_bsp_DelayMS(2);				//FOR 9320,延时2us	    
...
{ 					
	...
	readWriteTiming.FSMC_AddressSetupTime = 0x15;	 //地址建立时间（ADDSET）为2个HCLK 1/36M=27ns
    readWriteTiming.FSMC_AddressHoldTime = 0x00;	 //地址保持时间（ADDHLD）模式A未用到	
    readWriteTiming.FSMC_DataSetupTime = 0x15;		 // 数据保存时间为16个HCLK,因为液晶驱动IC的读数据的时候，速度不能太快，尤其对1289这个IC。
	...
	writeTiming.FSMC_AddressSetupTime = 0x15;	 //地址建立时间（ADDSET）为1个HCLK  
    writeTiming.FSMC_AddressHoldTime = 0x15;	 //地址保持时间（A		
    writeTiming.FSMC_DataSetupTime = 0x15;		 ////数据保存时间为4个HCLK	
	...
	bsp_DelayMS(50); 					// bsp_DelayMS 50 ms 
	...
	}else if(lcddev.id==0x5510)
	{
		...
		LCD_WriteReg(0xBC02,0x00);
		//Timing control 4H w/ 4-bsp_DelayMS
...
{   
	...
	fill_color_used_dma(0,0,&fillData,totalpoint);
} 
//在指定区域内填充单个颜色
...
{          
	...
	fillData = color;
	{
		...
		for(i=sy;i<= ey + sy;i++)
		{
			fill_color_used_dma(sx,i,&fillData,ex);
		}
	}	 
} 
...
void LCD_Clear_Area(u16 xsta, u16 ysta,u16 xend,u16 yend,u16 color)
{
    u16 i,j;
	u16 xlen=0;
#if USE_HORIZONTAL
...
void LCD_Show_Bar(u16 x, u16 y,u16 barhigh,u16 barwidth,u32 val,u32 valtotal,u16 pencolor,u16 backcolor)
{
    u16 ysta,xsta;
    u8 step;

	 if(val>valtotal) val=valtotal;
	 step=(u8)(barwidth*val/valtotal);
     LCD_Clear_Area(x+step,y,x+barwidth-1,y+barhigh-1,backcolor);    //清空
     
 #if USE_HORIZONTAL     
     for(xsta=x;xsta<step+x;xsta++)
     {
         LCD_SetCursor(xsta,y);      //设置光标位置
         LCD_WriteRAM_Prepare();     //开始写入GRAM
         for(ysta=0;ysta<barhigh;ysta++) LCD_WR_DATA(pencolor);//设置光标位置
     }
 #else   
     for(ysta=y;ysta<barhigh+y;ysta++)
     {
         LCD_SetCursor(x,ysta);      //设置光标位置
         LCD_WriteRAM_Prepare();     //开始写入GRAM
         for(xsta=0;xsta<step;xsta++) LCD_WR_DATA(pencolor);//设置光标位置
     }
