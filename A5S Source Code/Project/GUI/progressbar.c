...
#define USER_DEFI_NUM	11	//用户自定义的数据个数
#define ITETM_NUM	16	//用户自定义的每个数据 所占用的bit
...
void progressbar_show_percnt(u16 x,u16 y,u8 dir,u8 num,u16 color)
{
	...
	savePiont = POINT_COLOR;
	POINT_COLOR = color;
	LCD_ShowxNum(x , y,num,len,font,1);
	LCD_ShowChar(x + (len )*(font/2), y,'%',font,1);
	POINT_COLOR = savePiont;
...
void progressbar_draw_sign(_progressbar_obj *prgbx,u16 x,u16 y)
{
	...
	{
		tempData = prgbx->curpos * 1.0;
		percnt=tempData/prgbx->totallen * 100;//得到百分数
		//DBG("len:%d,pos:%d,percnt:%d\r\n",prgbx->totallen,prgbx->curpos,percnt);
		if(prgbx->type&PRGB_TYPE_VER)//垂直的
		{
			if(prgbx->width<8&&prgbx->height<20)return;//无法显示完整的百分数,直接不显示.
			progressbar_show_percnt(prgbx->left+(prgbx->width-8)/2,prgbx->top+prgbx->height/2,1,percnt,prgbx->btncolor);
		}else  //水平的
		{
			if(prgbx->height<8&&prgbx->width<20)return;//无法显示完整的百分数,直接不显示.
			progressbar_show_percnt(prgbx->left + prgbx->width/2,prgbx->top+(prgbx->height-8)/2 - 6,0,percnt,prgbx->btncolor);
 		}
	}
}
