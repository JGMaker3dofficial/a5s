...
void gui_init(void)
{
	...
	gui_phy.fill=LCD_Fill;//使用带DMA的填充		 	 
	gui_phy.fill_dma =LCD_Fill_DMA;
...
void gui_fill_rectangle_DMA(u16 x0,u16 y0,u16 width,u16 height,u16 color)
{	  							   
	if(width==0||height==0)return;//非法.	 
	gui_phy.fill_dma(x0,y0,width,height,color);	   	   
}
...
////获取文件名长度
////比如*pnSr = "0:\GCODE\202020.gcode",文件名为202020
//u8 get_file_name_size(u8 *pnSr)
//{
//	u8 count = 0;
//	u8 *tempP = pnSr + 9;//跳过"0:\GCODE\"
//	while(*tempP !='.')
//	{
//		tempP++;
//		count++;
...
//提取处gcode文件名，去掉后缀.gcode
void get_file_name_no_suffix(u8 *pnDes)
{
	u8 count = 0;
	u8 i;
	u8* temp = pnDes;
...
void gui_draw_single_color_icos(u16 x,u16 y,u16 width,u16 height,u8 *index,u16 pen_color,u16 back_color)
{
    ...
    W25QXX_Read(icon_buf,ICON_START_ADDR+*(u32*)index,6);
				...
				for(j=0;j<height;j++)	//列循环
					{
						for(i=0;i<width;i++)//写一行像素
							 {  	
								 ...
								 offset=i%8;
								 if(offset==0)								 
								 W25QXX_Read(icon_buf,sum+group,1);
