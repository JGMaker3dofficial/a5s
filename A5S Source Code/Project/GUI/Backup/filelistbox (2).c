...
void filelistbox_show_path(_filelistbox_obj *filelistbox)
{
	...
	gui_draw_icosalpha(filelistbox->left+5,filelistbox->top+(FLBOX_PATH_HEIGHT-16)/2,16,offset);//偏移5个单位,显示图标
	offset=0;
	if(path==NULL)return;
...
u8 filelistbox_scan_filelist(_filelistbox_obj *filelistbox)
{
 	...
 	if(filelistbox->path)	//路径有效
	{
		...
		while(res==FR_OK)
		{
			...
			switch (flboxfinfo.fattrib)
			{
			DEBUG_PRINTF("*file atribute:0x%x\r\n",flboxfinfo.fattrib);
				case AM_RDO:DEBUG_PRINTF("@:Read only\r\n");
				case AM_ARC:DEBUG_PRINTF("@:Archive\r\n");	
				case AM_HID: DEBUG_PRINTF("@:Hidden\r\n");
					break;
				case AM_SYS:DEBUG_PRINTF("@:System\r\n");
					break;
				case AM_VOL:DEBUG_PRINTF("@:Volume label\r\n");
					break;
				case AM_LFN:DEBUG_PRINTF("@:LFN entry \r\n");
					break;
				case AM_DIR:DEBUG_PRINTF("@@@@:Directory\r\n");
					break;
				
					
				case AM_MASK:DEBUG_PRINTF("@:Mask of defined bits\r\n");
					break;						
				default:
					break;
			}
			...
			if(filelistbox_addlist(filelistbox,fn,type))break;//添加失败了,每找到一个目标文件就添加到文件列表中         
		}
