...
void window_draw(_window_obj * windowx)
{
	...
	if(windowx->type&(1<<5))gui_show_strmid(windowx->left,windowx->top,windowx->width,windowx->captionheight,windowx->captioncolor,windowx->font,windowx->caption,1);//显示标题
...
u8 window_msg_box(u16 x,u16 y,u16 width,u16 height,u8 *str,u8 *caption,u8 font,u16 color,u8 mode,u16 time)
{
	...
	if(((mode&0X03)==0)&&(mode&(1<<6)))bsp_DelayMS(time);//没有按钮,且需要读取背景色,则调用延时 
