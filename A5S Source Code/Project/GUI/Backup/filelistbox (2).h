...
__packed typedef struct 
{
	...
	u16 totalPageCnt;				//总共需要多少页来显示 本次遍历 的目标文件或者文件夹

	u16 itemPerPage;				//每页需要显示多少个目标文件或者文件夹，初始化设定
	u16 itemCnt;					//文件数目+文件夹数目
	u16 fliter;						//文件过滤器,大类过滤，初始化设置
									//参见"exfuns.h"定义
	u16 fliterex;					//文件扩展的过滤器//用来选择大类里面的小类,最多支持16种不同的文件.默认0XFFFF,即不对小类进行刷选.，初始化设置
	
	u16 pathDepth;					//文件目录深度
	
	u16 folderTotalItems;			//文件夹总数目
	u16 fileTotalItems;				//目标文件总数目
	
	u8* path;						//父目录路径
 	u16 *fileIndexTab;    				//本次遍历的目标文件+文件夹的索引表	
...
extern const u8* FLIST_DISK_NAME_TBL[2][GUI_LANGUAGE_NUM];
