...
{static u8 screen_key=0x00;//图标按键
	...
	{
		if(screen_key & BUTTON_DOWN)//之前按下
		{ screen_key &= BUTTON_UP_MASK;  //松开标志
			screen_key|=BUTTON_ACTION;	//标志为需要响应
			keydev[screen_key & BUTTON_SELECTION_MASK]->sta &= BUTTON_CLEAR_STATE;
			keydev[screen_key & BUTTON_SELECTION_MASK]->sta|=BTN_RELEASE;	//松开
